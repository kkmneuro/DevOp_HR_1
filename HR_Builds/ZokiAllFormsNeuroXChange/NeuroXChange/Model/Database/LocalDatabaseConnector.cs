...
namespace NeuroXChange.Model.Database
{
    public class LocalDatabaseConnector
    {
        ...
        public void WriteBioData(BioData.BioData data)
        {
            
            if (!saveBioData || emulationOnHistoryMode)
            {
                return ;
            }
            /*
            ...
            if (requestcount == 0)
            {
                biorequest = new BioDataRequest();                
                biorequest.Acc= Globals.AccountId;
                biorequest.msgtype = 61;
                biorequest.BDV = new List<BioDataValues>();

            }
            biorequest.NR = requestcount + 1;

            //DateTime timeBegin = new DateTime(1970, 1, 1);
            //DateTime currentDate = DateTime.UtcNow;
            //long elapsedTicks = currentDate.Ticks - timeBegin.Ticks;

            string currDate = DateTime.UtcNow.ToString();
            biorequest.BDV.Add(new BioDataValues { X = data.accX, Y = data.accY, Z = data.accZ, AS = data.applicationStates, BT = currDate, HB = data.heartRate, SC = data.skinConductance, Tmp = data.temperature, TS = data.trainingStep, TT = data.trainingType });

            if (requestcount==10)
            {
                biorequest.NR = biorequest.BDV.Count;
                var json = new JavaScriptSerializer().Serialize(biorequest);
                WsHelperOrders.INSTANCE.send(json);
                requestcount = 0;//reset count to 0
                biorequest.BDV.Clear();
            }
        ...
        BioDataRequest biorequest = new BioDataRequest();
        int requestcount = 0;
        List<BioDataValues> biolist =  new List<BioDataValues>();
