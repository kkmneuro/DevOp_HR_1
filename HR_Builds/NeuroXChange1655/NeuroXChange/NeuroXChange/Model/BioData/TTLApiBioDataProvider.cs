    ...
    public class TTLApiBioDataProvider : AbstractBioDataProvider
    {
            ...
            :base(model, localDatabaseConnector)
        {
            ...
            while (true)
            {
                ...
                {
                    DialogResult dr = MessageBox.Show(ex.Message, "Device initialize", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk);
                    if (dr == DialogResult.Cancel)
                    {
                        break;
                    }
                    string filePathName = String.Concat(AppDomain.CurrentDomain.BaseDirectory, @"3dparty\TTLLiveCtrl.dll");
